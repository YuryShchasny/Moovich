...
class ShortMovieInfoMapper
    @Inject
    constructor() {
        private fun mapDtoToEntity(shortMovieInfoDto: ShortMovieInfoDto): ShortMovieInfo =
            ShortMovieInfo(
                ...
                previewUrl =
                    shortMovieInfoDto.poster?.previewUrl
                        ?: "https://yastatic.net/s3/kinopoisk-frontend/common-static/img/projector-logo/placeholder.svg",
            )

        fun mapListDtoToListEntity(shortMovieInfoDto: List<ShortMovieInfoDto>) =
            shortMovieInfoDto.map {
                mapDtoToEntity(it)
            }
